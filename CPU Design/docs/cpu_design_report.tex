...
\begin{enumerate}
  ...
  \begin{center}
    \ttfamily git clone \url{\GitHubCloneURL}
  \end{center}
  \item \textbf{Enter} the repository folder (clone creates \texttt{220-A/}). If a folder name has spaces, quote the path in \texttt{cd}.
\end{enumerate}

\subsection*{Quick start (repo root --- \texttt{demo.sh})}
\noindent From \texttt{220-A/} (where \texttt{demo.sh} lives):
\begin{lstlisting}
cd 220-A
chmod +x demo.sh
./demo.sh
...
\noindent\texttt{./demo.sh} defaults to Hello World; optional arguments \texttt{fibonacci} or \texttt{timer}:
\begin{lstlisting}
./demo.sh fibonacci
./demo.sh timer
\end{lstlisting}

\subsection*{Manual build (inside \texttt{CPU Design/})}
\noindent The Makefile and \texttt{./cpu} live under \texttt{CPU Design/} (see repository \texttt{README.md}):
\begin{lstlisting}
cd "CPU Design"
make
...
\noindent Run these from \texttt{CPU Design/} after \texttt{make}:
\begin{lstlisting}
...
\begin{table}[h!]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{2.35in} >{\raggedright\arraybackslash}p{4.1in}}
    ...
    \begin{minipage}[t]{\linewidth}\raggedright
      \begin{itemize}\setlength\itemsep{0.05em}
        \item Primary draft of \textbf{Programs} (Hello, Fibonacci, Timer) and the \textbf{Fibonacci demo video} instructions; checked narrative against \texttt{programs/*.asm} and expected outputs; led \textbf{testing} (Hello/Fibonacci/Timer runs, \texttt{--trace}, \texttt{--dump}) so the report matches observed behavior.
        \item Coordinated \textbf{Overleaf} structure and final PDF export; contributed \textbf{memory layout} material in the report (64\,KB RAM, MMIO window, downward stack, \texttt{SP16} eight-byte header and load address) consistent with \texttt{README.md} and \texttt{docs/isa.md}.
      \end{itemize}
    \end{minipage} \\
    \bottomrule
  \end{tabular}
